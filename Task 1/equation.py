...
def generate_equation(max_number):
    """
    Generate and return random equation in the format [NUMBER] [OPERATOR] x = [NUMBER].

    Returns a tuple with the equation as a string (for display purposes) and a list (for solving).
    """
    operators = ['+', '-', '*', '/']
    # Get random numbers and random operator from operators list to be used in the equation
    random_number_1 = random.randint(1, max_number)
    random_number_2 = random.randint(1, max_number)
    random_operator = random.choice(operators)
    # Create an equation array with the random values, for use when solving the equation
    equation_arr = [random_number_1,
                    random_operator, 'x', '=', random_number_2]
